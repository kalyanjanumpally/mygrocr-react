...
const ComponentManagePhoneOrders = () => {
	...
	//	const pendingOrdersListUrl = "http://localhost:8083/api/pending-orders/" + orderSourceType; 
		...
		const fetchPendingPhoneOrdersData = async () => {
			try {
	...
	const handleSearch = () => {
		...
		if(search.length > 0 ) {		
			...
			//	const findPhoneOrderByIdUrl = "http://localhost:8083/api/find-order-by-orderid/" + orderSourceType + "/"  + search; 

				const findPhoneOrderByIdUrl = APIs.findPhoneOrderByIdUrl + orderSourceType + "/"  + search; 
				
				const fetchPhoneOrderByIdData = async () => {
					try {
						const response = await fetch(findPhoneOrderByIdUrl);		
						...
						alert("From adminManagePhone: fetching orders, Server-side error. Please try later.")
					}			
				}			
				fetchPhoneOrderByIdData(); 	
			}
...
<>
	...
	<div className="container">	 
				...
				<div className="row ml-10 mt-20 mb-10">
					<label style={{fontSize:"30px"}}> Phone Orders </label>
				</div>	
...
export default ComponentManagePhoneOrders;